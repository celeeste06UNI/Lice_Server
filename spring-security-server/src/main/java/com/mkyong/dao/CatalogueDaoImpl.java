...
public class CatalogueDaoImpl implements CatalogueDao {
	...
	public void deleteCatalogue(Integer id_catalogue) {
		Catalogue catalogue = (Catalogue) sessionFactory.getCurrentSession().load(Catalogue.class, id_catalogue);
		if (null != catalogue) {
	...
	public List<Catalogue> getCatalogue(Integer id_catalogue) {
		...
		list = sessionFactory.getCurrentSession().createQuery("from Catalogue where id_catalogue=" + id_catalogue)
				.list();
		return list;
	}
	...
	public List<RuleProjCatalogue> getRuleProjCatalogue(Integer id_catalogue) {
		...
		list = sessionFactory.getCurrentSession()
				.createQuery("from RuleProjCatalogue where id_catalogue=" + id_catalogue).list();
		return list;
	...
	public List<RuleProjCatalogue> getListRuleProjCatalogue(Integer id_rule, Integer id_project) {
		...
		list = sessionFactory.getCurrentSession()
				.createQuery(
						"from RuleProjCatalogue where id_rule='" + id_rule + "' and id_project='" + id_project + "'")
				.list();
		return list;
	}
	...
	public List<Catalogue> getCatalogues(Integer id_rule, Integer id_project) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			aux = sessionFactory.getCurrentSession().createQuery("from Catalogue where id_catalogue=" + id_catalogue)
					.list();
	...
	public void deleteRuleProjCatalogue(Integer id_rule, Integer id_project, Integer id_catalogue) {
		...
		list = sessionFactory.getCurrentSession()
				.createQuery(
						"from RuleProjCatalogue where id_rule='" + id_rule + "' and id_project='" + id_project + "' and id_catalogue='" + id_catalogue + "'")
				.list();
		RuleProjCatalogue ruleProjCatalogue = list.get(0);
		if (null != ruleProjCatalogue) {
			this.sessionFactory.getCurrentSession().delete(ruleProjCatalogue);
		}
